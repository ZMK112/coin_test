...
from core.constant import OKX
from exchanges.rest import OKXRest
from models import QueryItem, OKXKlineRecord
from databases.clickhouse import ClickHouseClient, default_click
from core.logs import init_logger

LOGGER = init_logger('OKX')


class OKXCoin(OKXRest):
    exchange = OKX
    start_delay = 5
    request_limit = 200
    buffer_size = 20
    ...
    queue_size = 1000
    history_limit_range = 12 * 3600 * 1000
    kline_endpoint = 'api/v5/market/candles'
    history_kline_point = 'api/v5/market/history-candles'
    default_freq = '1m'
    ...
    def _generate_query_items(
        ...
        if freq not in self.valid_candle_intervals:
            raise ValueError(
                ...
                f"Supported: {list(self.valid_candle_intervals.keys())}"
            )
        if start >= end:
    ...
    async def query_candle_worker(self, query_item: QueryItem, endpoint: str):
        response = await self.query_minbar_worker(endpoint, query_item)
        ...
        candle = [OKXKlineRecord(symbol, exchange, dtype, freq, *r[:-1])
                  for r in response['data'] if r[-1] == '1']
        return candle
    ...
    async def query_symbol_candles(
        ...
        for query_item in query_items:
            try:
                ...
                if self.current_minute - query_item.open_time > self.history_limit_range:
                    endpoint = self.history_kline_point
    ...
    async def symbol_candles_with_save(
        ...
        if not candles:
            return
        self._last_bar[symbol] = candles[-1].open_time
        async with self._condition:
            await self.queue.put(candles)
            LOGGER.debug(f"{self.queue.qsize()}")
    ...
    async def _watcher(self):
        ...
        while not self._breaking:
            diff = self.current_minute - min(self._last_bar.values())
            LOGGER.debug(f"{diff=}")
            async with self._condition:
    ...
    async def symbol_candles_worker(self, symbol: str, dtype: str, freq: str):
        ...
        while not self._breaking:
            ...
            if delay_milliseconds >= bar_range * 1.3:
                await self.symbol_candles_with_save(
                    symbol=symbol,
                    dtype=dtype,
                    start=last_tick + bar_range - 1,
                    end=last_minute,
                    freq=freq
                )
            LOGGER.debug(
